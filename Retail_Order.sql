...
select * from (select * , row_number() over(partition by region order by sales desc) as rn from cte ) 
as Highest_selling_5products  where rn<= 5;

-- Q3.find month over month growth comparision for 2022 and 2023 sales eg. jan 2022 vs jan 2023 
...
select * from (select *, row_number () over (partition by category order by sales desc) as rn from cte ) as prime_month where rn=1;
